...
class RawSensorSource(private val context: Context) {
    ...
    /**
     * Unregister, stop the sensor thread, and wait for it — the single teardown both the
     * failed-start path and [stop] use.
     *
     * The join is the part that matters: `quitSafely()` only *requests* shutdown, running
     * callbacks already due and returning immediately. Callers close the recording right
     * after this returns, so without the wait an in-flight callback can arrive after the
     * writer is gone and be dropped. Both exit paths need that guarantee, which is why this
     * is one helper rather than two copies that drifted apart once already.
     */
    private fun shutdown(eventListener: SensorEventListener?, handlerThread: HandlerThread?) {
