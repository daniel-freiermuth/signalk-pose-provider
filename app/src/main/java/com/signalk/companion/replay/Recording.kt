...
object RecordingFormat {
    ...
    // collapse to the same null would quietly turn damage into a claim about the sensor.
    // parse() catches NumberFormatException and skips the whole line instead, which is what
    // the mandatory fields already do.
    private fun optF(s: String?): Float? = if (s == null || s == ABSENT) null else s.toFloat()
    private fun optD(s: String?): Double? = if (s == null || s == ABSENT) null else s.toDouble()
}
