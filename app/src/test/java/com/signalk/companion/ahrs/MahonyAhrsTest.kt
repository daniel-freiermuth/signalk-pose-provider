...
class MahonyAhrsTest {
    ...
    @Test
    fun `does not seed while the boat is swinging hard`() {
        // Regression: seeding checked only |a| ≈ g, so a first sample landing mid-tack could
        // seed from lever-arm acceleration that the correction path would have rejected. A
        // plausible magnitude pointing the wrong way is exactly the case the rate gate exists
        // for (P7).
        val f = MahonyAhrs(kp = 2f, ki = 0f)
        val (accel, mag) = sensorsAtRest(levelAtHeading(90f))
        f.onAccelerometer(accel[0], accel[1], accel[2])
        ...
        // First gyro sample arrives well above maxGyroForAccel.
        val hardSwing = f.maxGyroForAccel * 3f
        f.onGyroscope(1_000_000_000L, 0f, 0f, hardSwing)

        // Heading must still be the cold-start value, not the seeded 90°.
        assertDegreesNear(0f, angles(f).headingRad, 0.5f, "a hard swing must not seed heading")
    }
